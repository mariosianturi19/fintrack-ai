...
if (!failures.some((item) => item.name === "exact-size raster")) {
  pass("exact-size raster", "all declared 16, 24, 32, and 48 px files match");
}
if (!failures.some((item) => item.name === "8x raster inspection")) {
  pass("8x raster inspection", "all nearest-neighbor zoom files are exact 8x dimensions");
}

const sharedFont = path.resolve(ROOT, manifest.sharedFont.localAsset);
try {
  const fontStat = await fs.stat(sharedFont);
  if (fontStat.size > 0) {
    pass("shared font", "Space Grotesk source is available from the parent LOGO-R0 assets");
  } else {
    fail("shared font", sharedFont);
  }
} catch (error) {
  fail("shared font", `${sharedFont}: ${error.message}`);
}
